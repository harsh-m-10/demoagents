...
if page == "JD Creator":
    ...
    if st.button("Generate JD", type="primary"):
        ...
        if result["bias_report"]:
            st.subheader("Bias Check Findings")
            ...
            for i, candidate in enumerate(result["shortlist"], 1):
                ...
                with st.expander(f"#{i} — {r.get('name', 'Unknown')} | Score: {s.get('score', 'N/A')}/100 | {s.get('recommendation', 'N/A')}"):
                    ...
                    with col1:
                        ...
                        st.write(f"Current Role: {r.get('current_role', 'N/A')}")
                        st.write(f"Experience: {r.get('years_of_experience', 0)} years")
                        st.write(f"Skills: {', '.join(r.get('skills', []))}")
                    with col2:
...
# ================================================================
    ...
    with tab1:
        ...
        if st.button("Generate Offer Letter", type="primary"):
            ...
            if not result["budget_check"]["within_budget"]:
                st.error(f"Rejected: {result['budget_check']['message']}")
            else:
    ...
    with tab2:
        ...
        if st.button("Negotiate", type="primary"):
            ...
            # Decision badge
            st.subheader(f"Decision: {neg_result['decision']}")
...
# ================================================================
    ...
    if st.button("Run Full Pipeline", type="primary"):
        ...
        if jd_result["bias_report"]:
            st.warning(f"{len(jd_result['bias_report'])} bias issue(s) found")
        else:
